...
#
# Return:
# Returns 0 if tag is valid; returns 22 otherwise.
...
function parse_and_add_trailer()
{
  ...
  if [[ ! "$tag" ]]; then
    ...
    tag="$(str_strip "$tag")"
    if [[ "$keyword" != 'FIXES' ]] && ! is_valid_tag "$tag"; then
      return 22 # EINVAL
    fi
    parsed_trailer+="$tag"
  fi
...
function tag_help()
{
  ...
  printf '%s\n' 'kw tag:' \
    '  tag (--add-signed-off-by | -s) (<name>,...) [<patchset> | <sha>] - Add Signed-off-by' \
    '  tag (--add-reviewed-by | -r) (<name>,...) [<patchset> | <sha>] - Add Reviewed-by' \
    '  tag (--add-acked-by | -a) (<name>,...) [<patchset> | <sha>] - Add Acked-by' \
    '  tag (--add-tested-by | -t) (<name>,...) [<patchset> | <sha>] - Add Tested-by' \
    '  tag (--add-co-developed-by | -C) (<name>,...) [<patchset> | <sha>] - Add Co-developed-by and Signed-off-by' \
    '  tag (--add-reported-by | -R) (<name>;Closes|Link=<link>,...) [<patchset> | <sha>] - Add Reported-by' \
    '  tag (--add-fixes | -f) [<fixed-sha>] [<patchset> | <sha>] - Add Fixes' \
    '  tag (--verbose) - Show a detailed output'
}
